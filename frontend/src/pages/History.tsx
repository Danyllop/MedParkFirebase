...
const History = () => {
    ...
    useEffect(() => {
        const loadHistory = () => {
            try {
                const saved = localStorage.getItem('gate_a_history');
                if (saved) {
                    setHistoryData(JSON.parse(saved));
                } else {
                    setHistoryData(mockHistoryData as HistoryEntry[]);
                }
            } catch (error) {
    ...
    }, [historyData, searchTerm, typeFilter]);
            ...
            <div className="flex-1 overflow-hidden glass-card bg-slate-900/40 border border-white/5 rounded-2xl shadow-2xl flex flex-col">
                ...
                <div className="px-6 py-4 bg-slate-900 border-t border-white/5 flex items-center justify-between">
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                        Exibindo <span className="text-white">{filteredHistory.length}</span> resultados de <span className="text-white">{historyData.length}</span>
                    </p>
