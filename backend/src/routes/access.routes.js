...
router.post('/entry', async (c) => {
    try {
        const { vacancyId, spot, ownerName, plate, vehicleId, contractorId, employeeId, contractorVehicleId, destination } = await c.req.json();
        ...
        if (plate)
            activeConditions.push({ plate, exitTime: null });
        if (employeeId)
            activeConditions.push({ employeeId, exitTime: null });
        if (contractorId)
            activeConditions.push({ contractorId, exitTime: null });
        if (vehicleId)
            activeConditions.push({ vehicleId, exitTime: null });
        ...
        const log = await prisma.accessLog.create({
            data: {
                ...
                entryTime: new Date(),
                destination,
            }, // Cast to any because TS might not see new schema fields yet
        });
        return c.json(log, 201);
    }
    catch (error) {
        return c.json({ error: 'Erro ao registrar entrada.', details: error.message }, 500);
    }
});
// POST /v1/access/exit
router.post('/exit', async (c) => {
    try {
        const { vacancyId, spot, ownerName, plate } = await c.req.json();
        // Liberar vaga
        if (vacancyId) {
            await prisma.vacancy.update({
                where: { id: vacancyId },
        ...
        return c.json({ error: 'Erro ao registrar saída.', details: error.message }, 500);
    }
});
// GET /v1/access/history
router.get('/history', async (c) => {
    try {
        const { spot, plate, event, limit = '200', offset = '0' } = c.req.query();
        const where = {};
        if (spot)
            where.spot = { contains: spot.toUpperCase() };
        if (plate)
        ...
        return c.json({ data: logs, total, limit: parseInt(limit), offset: parseInt(offset) });
    }
    catch (error) {
        return c.json({ error: 'Erro ao buscar histórico.', details: error.message }, 500);
    }
});
